...
import { SubWorkflowCanvasHeader } from "./header";
...
function SubWorkflowCanvasInner({ nodeId }: SubWorkflowCanvasInnerProps) {
  ...
  const { onNodeDragStart, onNodeDrag, onNodeDragStop } = useSubWorkflowHoverOpen({
    ...
    moveIntoSubWorkflow: moveSubNodeIntoNestedSubWorkflow,
    openSubWorkflow,
  });
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col bg-zinc-950">
      <SubWorkflowCanvasHeader
        workflowName={workflowName}
        subWorkflowStack={subWorkflowStack}
        nodeCount={subNodes.length}
        edgeCount={subEdges.length}
        onBack={handleCloseSubWorkflow}
        onNavigateToRoot={() => handleNavigateToBreadcrumb(-1)}
