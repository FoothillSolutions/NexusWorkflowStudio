  ...
  return (
    <BaseNode accentHex={accentHex} selected={selected} label={data.label || displayName} type={data.type} icon={icon}>
      <div className="flex flex-col gap-2">
        {data.switchExpr && (
          <div className="font-mono text-xs text-zinc-300 bg-zinc-950/50 p-2 rounded border border-zinc-800 break-words">switch ({data.switchExpr})</div>
        )}
        <div className="flex flex-col gap-1">
          {data.cases.map((c, i) => <div key={i} className="text-[10px] text-zinc-500 truncate">• {c}</div>)}
      ...
      {allHandles.map((label, i) => {
        ...
        return (
          <div key={handleId} className="absolute flex items-center" style={{ top: `${topPercent}%`, right: -12, transform: "translateY(-50%)" }}>
            <span className="mr-1 text-[10px] text-zinc-400 whitespace-nowrap">{label}</span>
            <Handle type="source" position={Position.Right} id={handleId} className={`!relative !right-0 !transform-none ${HANDLE_CLASS}`} style={{ backgroundColor: accentHex }} />
          </div>
        );
      })}
    </BaseNode>
